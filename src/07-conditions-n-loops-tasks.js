...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startSymbol}${interval}${endSymbol}`;
}
...
function evaluateTicTacToePosition(position) {
  ...
  const isWinningCombination = (combination) => {
    ...
    return (
      ...
      && position[row1][col1] === position[row3][col3]
    );
  };

  for (let i = 0; i < winningCombinations.length; i += 1) {
    if (isWinningCombination(winningCombinations[i])) {
      const [p1] = winningCombinations[i];
      const [row, col] = p1;
